...
int intel_hfi_get_ipcc_baseline(void)
{
	...
	do {
		...
		baseline = hfi_ipcc_baseline;
	} while (read_seqcount_retry(&hfi_ipcc_seqcount, seq));

	return baseline ? baseline : 1;   /* nunca 0, evita divisão por zero antes da 1ª população */
}
...
static void set_hfi_ipcc_scores(struct hfi_instance *hfi_instance)
{
	...
	for_each_cpu(cpu, hfi_instance->cpus) {
		/*
		 * Baseline = average perf_cap across classes of the first
		 * P-core found (reference "P-core 0"). Computed once, on the
		 * first full population pass, then frozen.
		 */
		bool is_baseline_ref = !hfi_ipcc_baseline_set && !got_baseline_ref &&
					cpu_data(cpu).topo.cpu_type != INTEL_CPU_TYPE_ATOM;
		unsigned int sum = 0;
		int c, *scores;
		s16 index;
		...
		if (is_baseline_ref) {
			hfi_ipcc_baseline = sum / hfi_features.nr_classes;
			got_baseline_ref = true;
		}
	}

	if (!hfi_ipcc_baseline_set) {
		hfi_ipcc_baseline_set = true;
		pr_info("IPCC BASELINE: %d (média de perf_cap das classes do P-core de referência)\n",
			hfi_ipcc_baseline);
	}
...
{
	pr_info("IPCC: entrando em hfi online");
	struct hfi_instance *hfi_instance;
	struct hfi_cpu_info *info;
	u16 pkg_id;
	phys_addr_t hw_table_pa;
	...
	if (cpu_feature_enabled(X86_FEATURE_ITD)) 
	{
		pr_info("IPCC: X86_FEATURE_ITD ATIVADO");
