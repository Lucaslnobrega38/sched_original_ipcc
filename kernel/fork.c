					...
					struct kernel_clone_args *args)
{
	...
	if (p->mm) {
		...
		if (ipcc_odf_active() && current->mm) {
			/* Link before the write-protect pass: the target's
			 * first COW fault can land the moment it resumes.
			 */

			 // ordem deveria importar? 
			ipcc_odf_link_shadow(p->mm, current->mm);
			ipcc_odf_wrprotect_target(current->mm);
		}
	}
	retval = copy_namespaces(clone_flags, p);
	if (retval)
